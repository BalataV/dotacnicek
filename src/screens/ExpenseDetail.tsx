  ...
  if (!e) {
    return (
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Text onPress={() => actions.navigate('group')} style={{ color: c.onbg, fontFamily: FONTS.body800, fontSize: 15 }}>{t('‹ Zpět')}</Text>
        <Text style={{ fontFamily: FONTS.body700, color: c.onbg, marginTop: 12 }}>{t('Výdaj nenalezen.')}</Text>
      </ScrollView>
    );
  }

  const uneven = !!(e.shares && e.shares.length === e.parts.length); // nerovné dělení
  const splitLabel = e.splitType === 'ratio' ? t('Poměrově') : e.splitType === 'exact' ? t('Podle cen') : t('Rovným dílem');

  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
      <Text onPress={() => actions.navigate('group')} accessibilityRole="button" suppressHighlighting style={{ color: c.onbg, fontFamily: FONTS.body800, fontSize: 15, marginBottom: 10 }}>{t('‹ Zpět')}</Text>
      ...
      <View style={{ alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: categoryOf(e.category).color, borderWidth: 2, borderColor: c.ink, borderRadius: 20, paddingVertical: 4, paddingHorizontal: 11, marginBottom: 16 }}>
        ...
        <Text style={{ fontFamily: FONTS.display600, fontSize: 13, color: '#fff' }}>{t(categoryOf(e.category).label)}</Text>
      </View>
      ...
      <View style={{ position: 'relative', marginBottom: 12 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 14, padding: 12 }}>
          <Avatar name={t(e.payer)} initial={initial(e.payer)} color={colorForMember(e.payer)} size={36} fontSize={13} />
          <Text style={{ flex: 1, fontFamily: FONTS.body800, fontSize: 15, color: c.ink }}>{t('Platil ') + t(e.payer)}</Text>
        </View>
      </View>
      ...
      <View style={{ position: 'relative', marginBottom: 20 }}>
        ...
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 14, overflow: 'hidden' }}>
          <Text style={{ fontFamily: FONTS.body800, fontSize: 11, color: c.muted, textTransform: 'uppercase', letterSpacing: 0.5, padding: 12, paddingBottom: 8 }}>{t('Rozděleno mezi')} · {splitLabel}{!uneven ? ' · ' + fmtMoney(e.amount / e.parts.length, e.currency) + t('/os') : ''}</Text>
          {e.parts.map((p, idx) => (
            <View key={p} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, paddingHorizontal: 12, borderTopWidth: 2, borderTopColor: 'rgba(0,0,0,0.07)' }}>
              <Avatar name={t(p)} initial={initial(p)} color={colorForMember(p, idx)} size={28} />
              <Text style={{ flex: 1, fontFamily: FONTS.body700, fontSize: 14, color: c.ink }}>{t(p)}</Text>
      ...
      <Pushable onPress={() => actions.startEdit(e.id)} radius={14} style={{ marginBottom: 10 }}>
        <View style={{ backgroundColor: c.accent, borderWidth: 3, borderColor: c.ink, borderRadius: 14, paddingVertical: 14, alignItems: 'center' }}>
          <Text style={{ fontFamily: FONTS.display600, fontSize: 16, color: '#fff' }}>{t('✏️ Upravit výdaj')}</Text>
        </View>
      </Pushable>
      ...
      <Pushable onPress={() => actions.deleteExpense(e.id)} radius={14}>
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 14, paddingVertical: 14, alignItems: 'center' }}>
          <Text style={{ fontFamily: FONTS.display600, fontSize: 16, color: c.bad }}>{t('Smazat výdaj')}</Text>
        </View>
      </Pushable>
    </ScrollView>
  );
