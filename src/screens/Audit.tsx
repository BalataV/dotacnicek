...
function rankTitle(i: number, score: number): { medal: string; title: string } {
  if (score === 0) return { medal: '🚌', title: t('Černý pasažér') };
  if (i === 0) return { medal: '🏆', title: t('Sponzor večera') };
  if (i === 1) return { medal: '🥈', title: t('Dotační náměstek') };
  if (i === 2) return { medal: '🥉', title: t('Pokladník') };
  return { medal: '•', title: t('Řadový občan') };
}
...
export default function Audit() {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 28 }}>
      <Text onPress={() => actions.openGroup(g.id)} accessibilityRole="button" suppressHighlighting style={{ color: c.onbg, fontFamily: FONTS.body800, fontSize: 15, marginBottom: 10 }}>{t('‹ Skupina')}</Text>
      <Text style={{ fontFamily: FONTS.display700, fontSize: 26, color: c.onbg, letterSpacing: -0.5, marginBottom: 14 }}>{t('📋 Audit NKÚ')}</Text>
      ...
      <View style={{ position: 'relative', marginBottom: 18 }}>
        ...
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 18, overflow: 'hidden' }}>
          ...
          <View style={{ backgroundColor: c.ink, paddingVertical: 12, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text style={{ fontFamily: FONTS.display700, fontSize: 14, color: c.card, letterSpacing: 0.3 }}>{t('NEJVYŠŠÍ KONTROLNÍ ÚŘAD')}</Text>
              <Text style={{ fontFamily: FONTS.body700, fontSize: 11, color: c.card, opacity: 0.8 }}>{t('Kontrolní závěr č. j. NKÚ/')}{caseNo}/26</Text>
            </View>
            <View style={{ transform: [{ rotate: '-9deg' }], borderWidth: 2.5, borderColor: c.bad, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 }}>
              <Text style={{ fontFamily: FONTS.display700, fontSize: 12, color: c.bad, letterSpacing: 0.5 }}>{t('PROVĚŘENO')}</Text>
            </View>
          </View>

          <View style={{ paddingVertical: 8, paddingHorizontal: 14 }}>
            <Text style={{ fontFamily: FONTS.body800, fontSize: 12, color: c.muted }}>{t('Kontrolovaný subjekt')}</Text>
          ...
          {empty ? (
            <Text style={{ fontFamily: FONTS.body700, fontSize: 14, color: c.muted, paddingHorizontal: 14, paddingBottom: 16 }}>
              {t('Žádné prostředky k prověření. Kontrola pozastavena. Makáme.')}
            </Text>
          ) : (
            <>
              <Para no="1" label={t('Celkový objem prostředků')} value={totalText} />
              <Para no="2" label={t('Počet proplacených transakcí')} value={String(count)} />
              <Para no="3" label={t('Průměr na hlavu')} value={fmtMoneyMap(perHead(total, g.members.length))} />
              {big && <Para no="4" label={t('Největší jednorázová položka')} value={big.desc + ' · ' + fmtMoney(big.amount, big.currency)} />}
            </>
          )}
        </View>
      </View>
      ...
      {!empty && cats.length > 0 && (
        <>
          <Text style={{ fontFamily: FONTS.display600, fontSize: 18, color: c.onbg, marginBottom: 10 }}>{t('📊 Struktura výdajů')}</Text>
          <View style={{ position: 'relative', marginBottom: 18 }}>
                ...
                return (
                  <View key={slice.key} style={{ marginBottom: i < cats.length - 1 ? 12 : 0 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                      <Text style={{ fontFamily: FONTS.body800, fontSize: 13, color: c.ink }}>{cat.icon} {t(cat.label)}</Text>
      ...
      {!empty && (
        <>
          <Text style={{ fontFamily: FONTS.display600, fontSize: 18, color: c.onbg, marginBottom: 10 }}>{t('🏆 Žebříček sponzorů')}</Text>
          {board.map((row, i) => {
            ...
            return (
              <View key={row.name} style={{ position: 'relative', marginBottom: 10 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 11, backgroundColor: isTop ? c.bg2 : c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 14, paddingVertical: 11, paddingHorizontal: 12 }}>
                  ...
                  <Avatar name={t(row.name)} initial={initial(row.name)} color={colorForMember(row.name, i)} size={36} fontSize={13} />
                  <View style={{ flex: 1, minWidth: 0 }}>
                    <Text style={{ fontFamily: FONTS.body800, fontSize: 15, color: c.ink }} numberOfLines={1}>{t(row.name)}</Text>
                    <Text style={{ fontFamily: FONTS.body700, fontSize: 12, color: c.muted }}>{rt.title}{t(' · spotřeboval ')}{fmtMoneyMap(shares[row.name] || {}) || '0'}</Text>
                  </View>
          ...
          <Text style={{ fontFamily: FONTS.body700, fontSize: 12, color: c.onbg, opacity: 0.6, textAlign: 'center', marginTop: 14, lineHeight: 18 }}>
            {t('Kontrolu provedl revizní orgán. Pochybení nebylo shledáno.')}{'\n'}{t('My tam ty peníze máme. Sorry jako.')}
          </Text>
        </>
      )}
    </ScrollView>
