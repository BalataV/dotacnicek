...
function confirmDelete(onConfirm: () => void) {
  Alert.alert(
    t('Smazat účet?'),
    t('Trvale se smažou tvoje skupiny, výdaje i přihlašovací údaje. Tuto akci nelze vrátit.'),
    [
      { text: t('Zrušit'), style: 'cancel' },
      { text: t('Smazat účet'), style: 'destructive', onPress: onConfirm },
    ],
  );
}
  ...
  return (
    <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingVertical: 20, paddingBottom: 24, alignItems: 'center' }} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
      ...
      <View style={{ flexDirection: 'row', gap: 8, marginTop: 18, marginBottom: 16, width: '100%' }}>
        <StatBox value={fmtMoneyMap(owe)} label={t('Dlužíš')} color={c.bad} onPress={() => actions.navigate('overview')} />
        <StatBox value={fmtMoneyMap(owed)} label={t('Dostaneš')} color={c.good} onPress={() => actions.navigate('overview')} />
        <StatBox value={String(state.groups.length)} label={t('Skupiny')} color={c.accent} onPress={() => actions.navigate('overview')} />
      </View>
      ...
      <View style={{ width: '100%', position: 'relative', marginBottom: 16 }}>
        ...
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 18, padding: 14 }}>
          <Text style={{ fontFamily: FONTS.body800, fontSize: 11, color: c.muted, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Tvoje jméno')}</Text>
          {editName === null ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              <Text style={{ flex: 1, fontFamily: FONTS.display600, fontSize: 18, color: c.ink }} numberOfLines={1}>{t(state.myName)}</Text>
              <Pushable onPress={() => setEditName(state.myName === 'Já' ? '' : state.myName)} offset={2} radius={11}>
                <View style={{ backgroundColor: c.accent, borderWidth: 3, borderColor: c.ink, borderRadius: 11, paddingVertical: 8, paddingHorizontal: 14 }}>
                  <Text style={{ fontFamily: FONTS.display600, fontSize: 13, color: '#fff' }}>{t('✏️ Upravit')}</Text>
                </View>
              </Pushable>
            </View>
          ) : (
            <View>
              <Field value={editName} onChangeText={setEditName} placeholder={t('Napiš svoje jméno')} autoFocus maxLength={30} returnKeyType="done" onSubmitEditing={saveName} style={{ marginBottom: 10 }} />
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <Pushable onPress={saveName} disabled={!editName.trim() || state.busy} radius={12} style={{ flex: 1 }}>
                  <View style={{ backgroundColor: c.good, borderWidth: 3, borderColor: c.ink, borderRadius: 12, paddingVertical: 11, alignItems: 'center' }}>
                    <Text style={{ fontFamily: FONTS.display600, fontSize: 15, color: '#fff' }}>{t('Uložit')}</Text>
                  </View>
                </Pushable>
                <Pushable onPress={() => setEditName(null)} offset={0} radius={12} style={{ flex: 0 }}>
                  <View style={{ borderWidth: 3, borderColor: c.ink, borderRadius: 12, paddingVertical: 11, paddingHorizontal: 16, alignItems: 'center' }}>
                    <Text style={{ fontFamily: FONTS.display600, fontSize: 15, color: c.ink }}>{t('Zrušit')}</Text>
                  </View>
                </Pushable>
              </View>
            </View>
      ...
      <View style={{ width: '100%', position: 'relative', marginBottom: 16 }}>
        ...
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 18, overflow: 'hidden' }}>
          <View style={{ padding: 14, borderBottomWidth: 2, borderBottomColor: 'rgba(0,0,0,0.07)' }}>
            <Text style={{ fontFamily: FONTS.body800, fontSize: 11, color: c.muted, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Vzhled')}</Text>
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <Pressable onPress={() => actions.setTheme('zluta')} style={{ flex: 1, padding: 12, borderWidth: 3, borderColor: c.ink, borderRadius: 12, alignItems: 'center', backgroundColor: isZ ? '#FFD60A' : c.card }}>
                <Text style={{ fontFamily: FONTS.display600, fontSize: 14, color: isZ ? '#15233B' : c.muted }}>{t('Žlutá')}</Text>
              </Pressable>
              <Pressable onPress={() => actions.setTheme('modra')} style={{ flex: 1, padding: 12, borderWidth: 3, borderColor: c.ink, borderRadius: 12, alignItems: 'center', backgroundColor: isM ? '#102A43' : c.card }}>
                <Text style={{ fontFamily: FONTS.display600, fontSize: 14, color: isM ? '#fff' : c.muted }}>{t('Modrá')}</Text>
              </Pressable>
              <Pressable onPress={() => actions.setTheme('tmava')} style={{ flex: 1, padding: 12, borderWidth: 3, borderColor: c.ink, borderRadius: 12, alignItems: 'center', backgroundColor: isD ? '#0F1A2C' : c.card }}>
                <Text style={{ fontFamily: FONTS.display600, fontSize: 14, color: isD ? '#F2E8CE' : c.muted }}>{t('Tmavá')}</Text>
              </Pressable>
            </View>
          </View>
          <View style={{ padding: 14, borderTopWidth: 2, borderTopColor: 'rgba(0,0,0,0.07)' }}>
            <Text style={{ fontFamily: FONTS.body800, fontSize: 11, color: c.muted, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Velikost obsahu')}</Text>
            <View style={{ flexDirection: 'row', gap: 10 }}>
              {([['small', 'Malý'], ['medium', 'Střední'], ['large', 'Velký']] as const).map(([key, label]) => {
                ...
                return (
                  <Pressable key={key} onPress={() => actions.setContentSize(key)} style={{ flex: 1, padding: 12, borderWidth: 3, borderColor: c.ink, borderRadius: 12, alignItems: 'center', backgroundColor: on ? c.accent : c.card }}>
                    <Text style={{ fontFamily: FONTS.display600, fontSize: key === 'small' ? 12 : key === 'large' ? 17 : 14, color: on ? '#fff' : c.muted }}>{t(label)}</Text>
                  </Pressable>
                );
              })}
            </View>
          ...
          <View style={{ padding: 14, borderTopWidth: 2, borderTopColor: 'rgba(0,0,0,0.07)' }}>
            <Text style={{ fontFamily: FONTS.body800, fontSize: 11, color: c.muted, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Jazyk')}</Text>
            <View style={{ flexDirection: 'row', gap: 10 }}>
              {([['cs', 'Čeština'], ['en', 'Angličtina']] as const).map(([key, label]) => {
                const on = state.lang === key;
                return (
                  <Pressable key={key} onPress={() => actions.setLang(key)} style={{ flex: 1, padding: 12, borderWidth: 3, borderColor: c.ink, borderRadius: 12, alignItems: 'center', backgroundColor: on ? c.accent : c.card }}>
                    <Text style={{ fontFamily: FONTS.display600, fontSize: 14, color: on ? '#fff' : c.muted }}>{t(label)}</Text>
                  </Pressable>
                );
              })}
            </View>
          </View>
          <Toggle label={t('Chceš dostávat novinky z MF DNES')} value={state.toggles.notif} onPress={() => actions.toggleSet('notif')} />
        </View>
      </View>
      ...
      <Pushable onPress={actions.logout} radius={14} style={{ width: '100%' }}>
        <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 14, paddingVertical: 14, alignItems: 'center' }}>
          <Text style={{ fontFamily: FONTS.display600, fontSize: 16, color: c.bad }}>{t('Odhlásit se')}</Text>
        </View>
      </Pushable>
      ...
      <Text onPress={() => actions.navigate('privacy')} accessibilityRole="button" suppressHighlighting style={{ fontFamily: FONTS.body700, fontSize: 13, color: c.onbg, opacity: 0.6, marginTop: 18, textDecorationLine: 'underline' }}>
        {t('Zásady ochrany osobních údajů')}
      </Text>
