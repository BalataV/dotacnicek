...
export default function Activity() {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 28 }}>
      ...
      <Text style={{ fontFamily: FONTS.display700, fontSize: 26, color: c.onbg, letterSpacing: -0.5, marginBottom: 14 }}>{t('🕓 Historie')}</Text>
      ...
      {feed.length === 0 ? (
        <View style={{ position: 'relative' }}>
          ...
          <View style={{ backgroundColor: c.card, borderWidth: 3, borderColor: c.ink, borderRadius: 18, padding: 22, alignItems: 'center' }}>
            <Text style={{ fontFamily: FONTS.display700, fontSize: 17, color: c.ink, textAlign: 'center' }}>{t('Zatím se nic nestalo')}</Text>
            <Text style={{ fontFamily: FONTS.body700, fontSize: 14, color: c.muted, marginTop: 6, textAlign: 'center' }}>{t('Přidej výdaj a začne se psát historie.')}</Text>
          </View>
        </View>
      ) : feed.map((it) => {
        ...
        return (
          <View key={it.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 11, marginBottom: 11 }}>
            {/* ikona události */}
            {isPay ? (
              <Avatar name={t(it.actor)} initial={initial(it.actor)} color={colorForMember(it.actor)} size={38} fontSize={14} />
            ) : (
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              {isPay ? (
                <Text style={{ fontFamily: FONTS.body800, fontSize: 14, color: c.onbg }} numberOfLines={2}>
                  <Text style={{ color: c.good }}>{t(it.actor)}</Text>{t(' vyrovnal')}{it.actor === 'Já' ? t('i jsme') : ''}{t(' dluh u ')}<Text style={{ fontFamily: FONTS.display600 }}>{t(it.counterparty || '')}</Text>
                </Text>
              ) : (
                <Text style={{ fontFamily: FONTS.body800, fontSize: 14, color: c.onbg }} numberOfLines={2}>
                  <Text style={{ fontFamily: FONTS.display600 }}>{t(it.actor)}</Text>{t(' přidal')}{it.actor === 'Já' ? t('i jsme') : ''}{t(' výdaj „')}{it.desc}"
                </Text>
              )}
