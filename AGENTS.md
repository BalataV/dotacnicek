...
- **Detekce jazyka (2026-07-29, oprava):** `navigator.language` v React Native NEEXISTUJE a `Intl` na Hermesu vrací `en-US` i na česky nastaveném telefonu → původní detekce hlásila Čechům angličtinu (projevilo se anglickou pozvánkou ze sdílení). `deviceLocale()` v `i18n.ts` teď čte `NativeModules.I18nManager.localeIdentifier` (Android) / `NativeModules.SettingsManager.settings` (iOS) — RN core, takže OTA. Fallback při neznámém locale je **`cs`**, `sk` → `cs`.
  - `langChosen` v `AppState`: uložený jazyk se respektuje JEN když si ho uživatel vybral v Profilu. Bez toho by chybná autodetekce zůstala v AsyncStorage navždy. Musí být i v závislostech ukládacího `useEffect` (jinak se volba shodná s detekcí neuloží).
- **App Links / Universal Links (2026-07-29):** pozvánka je `https://dotacnicek.cz/join/?g=KÓD` (`landingJoinUrl` v `config.ts`). **Vlastní cesta `/join/` je povinná** — Android App Links filtrují jen podle cesty, ne podle query, takže filtr na kořeni by appce podstrčil i `/app/?token_hash=…` (potvrzení e-mailu, obnova hesla) a rozbil je. `parseJoinCode` bere kód z `join/KÓD` i z `?g=`, takže staré odkazy dál fungují. `docs/join/index.html` jen přesměruje na `/?g=KÓD` pro lidi bez appky.
  - Ověřovací soubory: `docs/.well-known/assetlinks.json` (SHA-256 z Play Console → App signing) a `docs/.well-known/apple-app-site-association` (Apple Team ID). **Obojí má zatím placeholder** a vyžaduje NOVÝ build (app.json `android.intentFilters` + `ios.associatedDomains`).
  - **Content-type AASA: VYŘEŠENO, Cloudflare netřeba.** GitHub Pages servíruje `apple-app-site-association` jako `application/octet-stream` (přípona chybí, vlastní hlavičky Pages neumí). Apple oficiálně chce `application/json`, ALE ověřeno 2026-07-29: jejich CDN si soubor stáhla a rozparsovala v pořádku — `https://app-site-association.cdn-apple.com/a/v1/dotacnicek.cz` vrací HTTP 200 s obsahem. Migraci na Cloudflare tedy nedělat, pokud se neukáže reálný problém na zařízení.
  - Ověřovací nástroje: Apple `https://app-site-association.cdn-apple.com/a/v1/dotacnicek.cz`, Google `https://digitalassetlinks.googleapis.com/v1/assetlinks:check?source.web.site=…&relation=…&target.android_app.package_name=…&target.android_app.certificate.sha256_fingerprint=…`. **Obě služby cachují ~10 min i déle** — po změně souboru chvíli vracejí starý obsah, to není chyba.
  - `assetlinks.json` obsahuje DVA otisky: Play App Signing klíč (z Play Console) a upload klíč (vytažený z AAB), aby App Links fungovaly i u testovacích buildů z EAS. Apple Team ID `5N974388GT`, `appID` musí být `TEAM_ID.bundleID` (jen Team ID nestačí).
- **Vydávání buildů (přání uživatele, 2026-07-29):** Když je potřeba nový nativní build, **spustit ho rovnou sám** (`eas build --platform android|ios --profile production --non-interactive --no-wait`), počkat a stáhnout artefakty do `builds/` (skript `scripts/fetch-builds.sh <android-id> <ios-id>`, složka je v `.gitignore`). Uživateli pak jen napsat, ať je nahraje do obchodů. Publikování do obchodů NEDĚLAT bez jeho pokynu.
- **Landing – horní lišta NESMÍ měnit výšku při scrollu:** `.topbar` je `position: sticky` v běžném toku; když `.stuck` zmenšovala padding (14→8 px), zkrátil se dokument, obsah poskočil, `scrollY` spadlo pod práh a třída se zase odebrala → nekonečné blikání kousek pod vrcholem stránky. Přepínat jen `box-shadow` + hystereze (24 px nahoru, 8 px dolů). NEVRACET změnu paddingu.
- **Výška lišt v appce:** spodní navigace používá `navBarBottom` v `Root.tsx` — na iOS `insets.bottom - 12` (plný inset dělal na iPhonu nápadně vysoký bílý pruh; 22 px pořád přesahuje ~21px home indicator), na Androidu `insets.bottom + 6` (gesture pill potřebuje celý inset).
