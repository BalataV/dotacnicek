...
- **App Links / Universal Links (2026-07-29):** pozvánka je `https://dotacnicek.cz/join/?g=KÓD` (`landingJoinUrl` v `config.ts`). **Vlastní cesta `/join/` je povinná** — Android App Links filtrují jen podle cesty, ne podle query, takže filtr na kořeni by appce podstrčil i `/app/?token_hash=…` (potvrzení e-mailu, obnova hesla) a rozbil je. `parseJoinCode` bere kód z `join/KÓD` i z `?g=`, takže staré odkazy dál fungují. `docs/join/index.html` jen přesměruje na `/?g=KÓD` pro lidi bez appky.
  ...
  - ⚠️ **Riziko GitHub Pages:** `assetlinks.json` se servíruje jako `application/json` (OK), ale `apple-app-site-association` je bez přípony → GitHub Pages posílá `application/octet-stream`. Apple oficiálně chce `application/json`; jejich CDN to obvykle spolkne, ale zaručené to není a GitHub Pages vlastní hlavičky nastavit neumí. Když Universal Links na iPhonu nezaberou, řešení je přesunout DNS na Cloudflare (zdarma) a Transform Rule přepsat `Content-Type`, případně hostovat `docs/` na Cloudflare Pages / Netlify (`_headers`). Ověřeno 2026-07-29 curlem.
